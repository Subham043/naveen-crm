...
import type { SalesTeamRevenueReportType } from "@/utils/types";
import { Table } from "@mantine/core";
import TableRowNotFound from "@/components/TableRowNotFound";
import { memo } from "react";

type SalesTeamRevenueReportTableProps = {
  salesTeamRevenueReports: SalesTeamRevenueReportType[];
  loading: boolean;
};

const SalesTeamRevenueReportRow = memo(
  ({
    period,
    total_revenue,
    total_cost,
...
function SalesTeamRevenueReportTable({
  loading,
  salesTeamRevenueReports,
}: SalesTeamRevenueReportTableProps) {
  return (
    <Table.ScrollContainer minWidth={800} p={undefined} m={undefined}>
      <Table highlightOnHover horizontalSpacing="md">
        ...
        <Table.Tbody>
          ...
          ) : salesTeamRevenueReports.length > 0 ? (
            salesTeamRevenueReports.map((item, index) => (
              <SalesTeamRevenueReportRow key={index} {...item} />
            ))
          ) : (
...
export default SalesTeamRevenueReportTable;
